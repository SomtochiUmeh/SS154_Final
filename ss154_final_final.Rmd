...
imputed_data <- data %>%
  ...
  group_by(country) %>%
  nest() %>%
  mutate(imputed_govt_educ = map(data, ~ mice(data = .x[, c("year", "govt_educ")], m = 5, method = "pmm", seed = 123, printFlag = FALSE))) %>%
  mutate(imputed_pol_stab = map(data, ~ mice(data = .x[, c("year", "pol_stab_est")], m = 5, method = "pmm", seed = 123, printFlag = FALSE))) %>%
  mutate(imputed_data = map2(imputed_govt_educ, imputed_pol_stab, ~bind_cols(complete(.x, action = "long", include = TRUE), complete(.y, action = "long", include = TRUE)))) %>%
...
# descr(merged_data[, c("year", "gdp_cap", "trade_per_gdp", "pol_stab_est", "govt_educ", "internet")])
...
abline(v = 2006, lty = 2)
# arrows(1994, 25, 1996, 25, length = .1)
# text(1990, 25, "Medical Marijuana\nLegalization",
#     cex = 0.8)
```
